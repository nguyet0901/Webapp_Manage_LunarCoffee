    ...
    public static class Common
    {
        ...
        private static async Task<bool> ExecuteBirthday(string token ,List<SMSZNSBrandName> smslist)
        {
            try
            {
                ...
                {
                    ds = await confunc.ExecuteDataSet("YYY_sp_Auto_Tool_Birthday" ,CommandType.StoredProcedure);
                }
                if (ds != null)
                {
        ...
        private static async Task<bool> ExecuteAppointment(string token ,List<SMSZNSBrandName> smslist)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    ds = await confunc.ExecuteDataSet("YYY_sp_Auto_Tool_RemindAppointment" ,CommandType.StoredProcedure);
                }
                if (ds != null)
                {
                    DataTable dtResult = ds.Tables[0];
        ...
        private static async Task<bool> ExecuteLog()
        {
            try
            {
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    await confunc.ExecuteDataTable("YYY_sp_Auto_Tool_Log" ,CommandType.StoredProcedure ,
            ...
            try
            {
                ...
                using (HttpClient clientautho = new HttpClient() { BaseAddress = new Uri(Comon.Global.sys_HookEInvoice.Invoice_Url) })
                {
                    ...
                    if (token != null && token != "")
                    {
                        using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                        {
                            await confunc.ExecuteDataTable("YYY_sp_Auto_ThirdPartToken" ,CommandType.StoredProcedure
