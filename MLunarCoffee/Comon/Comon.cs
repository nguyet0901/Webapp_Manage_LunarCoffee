...
namespace MLunarCoffee.Comon
{
    public static class Comon
    {
        ...
        //            dt =await  confunc.ExecuteDataTable ("[YYY_sp_CheckLimitBranch_UpLoad]", CommandType.StoredProcedure,
        ...
        //        DataTable dt = new DataTable ();
        //        using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase ()) {
        //            dt =await confunc.ExecuteDataTable ("[YYY_sp_CheckLimitBranch_SMS]", CommandType.StoredProcedure,
        //              "@BranchID", SqlDbType.Int, user.sys_branchID);
        //        }
        //        if (dt != null && dt.Rows.Count != 0) {
        //            if (Convert.ToInt32 (dt.Rows[0][0].ToString ()) > 0) {
                ...
                foreach (var dr in parrentFolder)
                {
                    if (!none_change.Contains(dr))
                    {
                        ...
                        foreach (var drsub in folders)
                        {
                            ...
                            using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                            {
                                dtFolder = await confunc.ExecuteDataTable("[YYY_sp_DATA_INSERT_FOLDER]" ,CommandType.StoredProcedure ,
                            ...
                            foreach (var drimage in images)
                            {
                                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                                {
                                    dtFolder = await confunc.ExecuteDataTable("[YYY_sp_DATA_INSERT_IMAGE]" ,CommandType.StoredProcedure ,
