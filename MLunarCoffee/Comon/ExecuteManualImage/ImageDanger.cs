    ...
    public class ImageDanger
    {
        ...
        private async void SaveImage()
        {
            ...
            using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
            {
                ...
                dt = await confunc.ExecuteDataTable("[MLU_sp_Move_ImageCustomer]" ,CommandType.StoredProcedure);
            }
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    await confunc.ExecuteDataTable("[MLU_sp_Move_ImageUpdate]" ,CommandType.StoredProcedure
                        ,"@CustID" ,SqlDbType.Int ,custid
                        ,"@Image" ,SqlDbType.NVarChar ,filename);
                }

                // UPDATE VTT_Customer SET Avatar='https://cdnvttimg.MUSLUNARCOFFEE.com/DEMONK/_AvatarCustomer/'+Avatar
        ...
        private async void InsertImageSQL(int custid ,string foldername ,string imagestring)
        {
            ...
            {
                DataTable dt = new DataTable();
                dt = await confunc.ExecuteDataTable("MLU_sp_Move_ImageToSQL" ,CommandType.StoredProcedure
                    ,"@CustID" ,SqlDbType.Int ,custid
        ...
        public async void ReduceImageGet()
        {
            ...
            {
                DataTable dt = new DataTable();
                dt = await confunc.ExecuteDataTable("MLU_sp_Reduce_GetFolder" ,CommandType.StoredProcedure);
        ...
        private async void ReduceImageFolder_Exe(string CustID ,string FolderID ,string FolderName ,List<string> listcust)
        {
            try
            {
                ...
                {
                    DataTable dt = new DataTable();
                    dt = await confunc.ExecuteDataTable("MLU_sp_Reduce_GetImg" ,CommandType.StoredProcedure
                    ...
                    foreach (var imgname in listcust)
                    {
                        if (!imgname.Contains(".webp"))
                        {
                            ...
                            if (imghave)
                            {
                                ...
                                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                                {
                                    await confunc.ExecuteDataTable("MLU_sp_Reduce_UpdateImg" ,CommandType.StoredProcedure
                                        ,"@FolderID" ,SqlDbType.Int ,FolderID
