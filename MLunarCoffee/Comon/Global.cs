    ...
    public class GlobalUser
    {
        ...
        public async Task<AuthorResponse> User_Authorize(string user ,string pass ,string tokenfcm ,string iplan)
        {
            try
            {
                ...
                {
                    DataSet ds = await connFunc.ExecuteDataSet("[YYY_sp_Permission_User_LogIn]" ,CommandType.StoredProcedure ,
    ...
    public class Global
    {
        ...
        public static async Task System_Start(string keycode ,IConfiguration _config)
        {
            try
            {
                ...
                APIKEY = _config.GetValue<string>("DATA:KEYNAME").ToString();
                APICODE = _config.GetValue<string>("DATA:KEYCODE").ToString();
                ROOTCODE = _config.GetValue<string>("DATA:ROOTCODE").ToString();
                CLIENTIP = Encrypt.DecryptString(_config.GetValue<string>("CLIENT:CIP").ToString() ,ROOTCODE);
                CLIENTNAME = Encrypt.DecryptString(_config.GetValue<string>("CLIENT:CNAME").ToString() ,ROOTCODE);
                CLIENTUSER = Encrypt.DecryptString(_config.GetValue<string>("CLIENT:CUSER").ToString() ,ROOTCODE);
                CLIENTPASSWORD = Encrypt.DecryptString(_config.GetValue<string>("CLIENT:CPASSWORD").ToString() ,ROOTCODE);
                ROOTIP = Encrypt.DecryptString(_config.GetValue<string>("ROOT:RIP").ToString() ,ROOTCODE);
                ROOTNAME = Encrypt.DecryptString(_config.GetValue<string>("ROOT:RNAME").ToString() ,ROOTCODE);
                ROOTUSER = Encrypt.DecryptString(_config.GetValue<string>("ROOT:RUSER").ToString() ,ROOTCODE);
                ROOTPASSWORD = Encrypt.DecryptString(_config.GetValue<string>("ROOT:RPASSWORD").ToString() ,ROOTCODE);
        ...
        private static async Task<int> Client_GetDBString(string db)
        {
            try
            {
                using (Models.ExecuteDataBaseRoot connFunc = new Models.ExecuteDataBaseRoot())
                {
                    ...
                    dt = await connFunc.ExecuteDataTable("YYY_sp_DB_DetectNameString" ,CommandType.StoredProcedure ,"@db" ,SqlDbType.NVarChar ,db);
                    if (dt != null && dt.Rows.Count != 0)
                    {
