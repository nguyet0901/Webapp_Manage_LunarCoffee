...
namespace MLunarCoffee.Comon
{
    public static class SendNoti
    {
        ...
        //        {
        //            dt =await connFunc.ExecuteDataTable("[MLU_sp_AppCustomer_Notification_Insert_Push]", CommandType.StoredProcedure,
        ...
        public static void SendSMS_OneToken(string CustomerID, string Amount)
        {
            try
            {
                if (Global.sys_isSMSAfterPayment == 1)
                {
                    ...
                    //using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    //{
                    //    dt =await connFunc.ExecuteDataTable("MLU_sp_SMS_GetPhoneCustomer", CommandType.StoredProcedure,
