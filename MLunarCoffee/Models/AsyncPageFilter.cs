    ...
    public class AsyncPageFilter : IAsyncPageFilter
    {
        ...
        public async Task OnPageHandlerExecutionAsync(PageHandlerExecutingContext context ,PageHandlerExecutionDelegate next)
        {
            ...
            if (context.HttpContext.Request.Method == "GET")
            {
                ...
                if (IsMinify != "1" && isPass && path == "/customer/maincustomer")
                {
                    var CusID = context.HttpContext.Request.Query["CustomerID"].ToString();
                    if (int.TryParse(CusID, out int n)) {
                        string EncryCus = Encrypt.EncryptString(CusID ,Settings.LittleSecret);
                        context.Result = new RedirectResult(String.Format("/Customer/MainCustomer?CustomerID={0}&ver={1}", EncryCus, Comon.Global.sys_DBVersion));
                        return;
                    }
                }
            }
