                ...
                if (currentid != "0")
                {
                    tasks.Add(Task.Run(async () =>
                    {
                        using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                        {
                            ...
                            dtInfo = await connFunc.ExecuteDataTable("[MLU_sp_Product_StockInDetail_Info]", CommandType.StoredProcedure
                    ...
                    tasks.Add(Task.Run(async () =>
                    {
                        using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                        {
                            ...
                            dtItems = await connFunc.ExecuteDataTable("[MLU_sp_Product_StockInDetail_Item]", CommandType.StoredProcedure
                           , "@ID", SqlDbType.Int, Convert.ToInt32(currentid));
