    ...
    public class RequireWareDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostInitialize(string currentid)
        {
            try
            {
                ...
                if (currentid != "0")
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dt = await connFunc.ExecuteDataTable("[MLU_sp_Product_OrderDetailWare_Info]", CommandType.StoredProcedure
                    ...
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dt = await connFunc.ExecuteDataTable("[MLU_sp_Product_OrderDetailWare_Item]", CommandType.StoredProcedure
                           , "@OrderID", SqlDbType.Int, Convert.ToInt32(currentid));
                        dt.TableName = "Items";
                        ds.Tables.Add(dt);
                    }
        ...
        public async Task<IActionResult> OnPostExcuteData(string currentid, string datamain, string datadetail)
        {
            try
            {
                ...
                if (currentid == "0")
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dtResult = await connFunc.ExecuteDataTable("MLU_sp_Product_OrderWare_Insert", CommandType.StoredProcedure,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dtResult = await connFunc.ExecuteDataTable("MLU_sp_v2_Product_OrderWare_Update", CommandType.StoredProcedure,
                            "@Note", SqlDbType.NVarChar, dataMain.Note != null ? dataMain.Note.Replace("'", "").Trim() : "",
        ...
        public async Task<IActionResult> OnPostDeleteItem(int id)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                {
                    dt = await connFunc.ExecuteDataTable("[MLU_sp_Product_OrderWare_Delete]", CommandType.StoredProcedure,
