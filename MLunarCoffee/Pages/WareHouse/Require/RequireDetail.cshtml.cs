    ...
    public class RequireDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostInitialize(string currentid, string ware)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                {
                    dt = await connFunc.ExecuteDataTable("[YYY_sp_User_LoadCombo]", CommandType.StoredProcedure);
                    dt.TableName = "User";
                    ds.Tables.Add(dt);
                }
                ...
                if (currentid != "0")
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dt = await connFunc.ExecuteDataTable("[YYY_sp_Product_OrderDetail_Info]", CommandType.StoredProcedure
                    ...
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dt = await connFunc.ExecuteDataTable("[YYY_sp_Product_OrderDetail_Item]", CommandType.StoredProcedure
                           , "@ID", SqlDbType.Int, Convert.ToInt32(currentid));
                        dt.TableName = "Items";
                        ds.Tables.Add(dt);
                    }
        ...
        public async Task<IActionResult> OnPostExcuteData(string data, string ware, string userto, string note, string CurrentID)
        {
            try
            {
                ...
                if (CurrentID == "0")
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dtResult = await connFunc.ExecuteDataTable("YYY_sp_v2_Product_Order_Insert", CommandType.StoredProcedure,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dtResult = await connFunc.ExecuteDataTable("YYY_sp_v2_Product_Order_Update", CommandType.StoredProcedure,
                            "@Note", SqlDbType.NVarChar, note.Replace("'", "").Trim(),
        ...
        public async Task<IActionResult> OnPostDeleteItem(int id)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                {
                    dt = await connFunc.ExecuteDataTable("[YYY_sp_Product_Order_Delete]", CommandType.StoredProcedure,
