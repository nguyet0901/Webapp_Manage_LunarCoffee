    ...
    public class ServiceList : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadInit()
        {
            try
            {
                ...
                {
                    dt = await confunc.ExecuteDataTable("YYY_sp_Service_UnitCount", CommandType.StoredProcedure);
                }
        ...
        public async Task<IActionResult> OnPostLoadataServiceType(int id)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataTable("YYY_sp_v2_Service_LoadType", CommandType.StoredProcedure
        ...
        public async Task<IActionResult> OnPostLoadataService(int id)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataTable("[YYY_sp_v2_Service_Load]", CommandType.StoredProcedure
                            , "@CurrentID", SqlDbType.Int, id
                        );
                }
                if (dt != null)
        ...
        public async Task<IActionResult> OnPostDisableItem(int id)
        {
            try
            {
                ...
                {
                    dt = await connFunc.ExecuteDataTable("[YYY_sp_Service_Disable]", CommandType.StoredProcedure,
                        "@CurrentID", SqlDbType.Int, id,
