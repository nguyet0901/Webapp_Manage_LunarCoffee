    ...
    public class CourseTabListModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadData(string Currentid)
        {
            try
            {
                ...
                {
                    ds = await confunc.ExecuteDataSet("[MLU_sp_ST_Settings_Course_LoadTabListDetail]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostDeleteItem(int courseid, int studentid)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataTable("[MLU_sp_ST_Course_DeteleStudent]", CommandType.StoredProcedure,
