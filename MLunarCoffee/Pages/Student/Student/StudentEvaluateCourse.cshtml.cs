...
{
    public class StudentEvaluateCourseModel : PageModel
    {
        public string _StudentID { get; set; }
        public void OnGet()
        {
            string StudentID = Request.Query["StudentID"];
            _StudentID = (StudentID != null) ? StudentID.ToString() : "0";
        }
        ...
        public async Task<IActionResult> OnPostInit()
        {
            try
            {
                ...
                {
                    dt = await confunc.ExecuteDataTable("[MLU_sp_ST_StatusLevel_LoadComBo]", CommandType.StoredProcedure);
                    dt.TableName = "StatusLevel";
                    ds.Tables.Add(dt);
                }
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataTable("[MLU_sp_ST_StatusRevi_LoadComBo]", CommandType.StoredProcedure);
                    dt.TableName = "StatusRevi";
                    ds.Tables.Add(dt);
                }
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataTable("[MLU_sp_ST_StatusCerti_LoadComBo]", CommandType.StoredProcedure);
        ...
        public async Task<IActionResult> OnPostLoadDetail(string studentid)
        {
            try
            {
                DataSet ds = new DataSet();
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    ds = await confunc.ExecuteDataSet("[MLU_sp_ST_Student_StudentStatus]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostExcuteData(string data)
        {
            try
            {
                ...
                {
                    dtResult = await connFunc.ExecuteDataTable("[MLU_sp_ST_Course_StudentStatus_Update]", CommandType.StoredProcedure,
