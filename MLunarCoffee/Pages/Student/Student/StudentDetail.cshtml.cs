    ...
    public class StudentDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadCombo()
        {
            try
            {
                ...
                tasks.Add(Task.Run(async () =>
                {
                    ...
                    {
                        DataTable dt = new DataTable();
                        dt = await confunc.ExecuteDataTable("[YYY_sp_ST_Settings_Source_LoadCombo]", CommandType.StoredProcedure);
                        dt.TableName = "Source";
                        return dt;
                    }
                }));
        ...
        public async Task<IActionResult> OnPostSearchCustomer(string TextSearch)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataTable("[YYY_sp_ST_Settings_Student_SearchCustomer]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostExecuted(int CurrentID, string data)
        {
            try
            {
                ...
                if (CurrentID == 0)
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dt = await connFunc.ExecuteDataTable("[YYY_sp_ST_Settings_Student_Insert]", CommandType.StoredProcedure
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dt = await connFunc.ExecuteDataTable("[YYY_sp_ST_Settings_Student_Update]", CommandType.StoredProcedure
                            , "@Avatar", SqlDbType.NVarChar, DataMain.Avatar
                            , "@Code", SqlDbType.NVarChar, DataMain.Code
                            , "@Name", SqlDbType.NVarChar, DataMain.Name
                            , "@Email", SqlDbType.NVarChar, DataMain.Email
        ...
        public async Task<IActionResult> OnPostLoadDetail(int CurrentID)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                {
                    dt = await connFunc.ExecuteDataTable("[YYY_sp_ST_Settings_Student_LoadDetail]", CommandType.StoredProcedure
