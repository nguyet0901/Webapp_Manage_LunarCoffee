    ...
    public class ExaminationDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadData(string currentid)
        {
            try
            {
                ...
                {
                    dt = await confunc.ExecuteDataTable("[MLU_sp_ST_Settings_Examination_LoadDetail]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostExcuteDataDetail(string data, string CurrentID)
        {
            try
            {
                ...
                if (CurrentID == "0")
                {
                    ...
                    {
                        dt = await connFunc.ExecuteDataTable("MLU_sp_ST_Settings_Examination_Insert", CommandType.StoredProcedure,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dt = await connFunc.ExecuteDataTable("MLU_sp_ST_Settings_Examination_Update", CommandType.StoredProcedure,
