    ...
    public class CertificateDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostExcuteData(string Name, string Avatar, string Note, string CurrentID)
        {
            try
            {
                ...
                if (CurrentID == "0")
                {
                    ...
                    {
                        dt = await connFunc.ExecuteDataTable("YYY_sp_ST_Certificate_Insert", CommandType.StoredProcedure,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dt = await connFunc.ExecuteDataTable("YYY_sp_ST_Certificate_Update", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostLoadDetail(string CurrentId)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                {
                    dt = await connFunc.ExecuteDataTable("YYY_sp_ST_Certificate_LoadDetail", CommandType.StoredProcedure,
                            "@CurrentId", SqlDbType.Int, Convert.ToInt32(CurrentId)
                            );
                }
                return Content(JsonConvert.SerializeObject(dt));
