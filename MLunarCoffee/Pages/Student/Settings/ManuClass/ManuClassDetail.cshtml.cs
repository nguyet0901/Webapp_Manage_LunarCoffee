    ...
    public class ManuClassDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadData(string currentid)
        {
            try
            {
                ...
                {
                    dt = await confunc.ExecuteDataTable("[YYY_sp_ST_Settings_ManuClass_LoadDetail]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostExcuteDataDetail(string data, string CurrentID)
        {
            try
            {
                ...
                if (CurrentID == "0")
                {
                    ...
                    {
                        dt = await connFunc.ExecuteDataTable("YYY_sp_ST_Settings_ManuClass_Insert", CommandType.StoredProcedure,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dt = await connFunc.ExecuteDataTable("YYY_sp_ST_Settings_ManuClass_Update", CommandType.StoredProcedure,
