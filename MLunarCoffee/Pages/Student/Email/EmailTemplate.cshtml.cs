    ...
    public class EmailTemplateModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostInitialize(string type)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataTable("MLU_sp_Mail_Multi_LoadBranch", CommandType.StoredProcedure
        ...
        public async Task<IActionResult> OnPostLoadData(string type, int masterID, int ID)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    ds = await confunc.ExecuteDataSet("MLU_sp_ST_Mail_LoadDataSend_V2", CommandType.StoredProcedure
                        , "@ID", SqlDbType.Int, ID
        ...
        public async Task<IActionResult> OnPostUpdateSend(int paymentID)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataTable("MLU_sp_ST_Mail_UpdateIsMail", CommandType.StoredProcedure
