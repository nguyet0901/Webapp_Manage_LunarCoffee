    ...
    public class TeacherDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadata(int id)
        {
            try
            {
                ...
                {
                    dt = await confunc.ExecuteDataTable("[MLU_sp_ST_Teacher_LoadDetail]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostExcuteData(string data, string CurrentID, string Avatar)
        {
            try
            {
                ...
                if (CurrentID.Trim() == "0")
                {
                    ...
                    {
                        dt = await connFunc.ExecuteDataTable("[MLU_sp_ST_Teacher_Insert]", CommandType.StoredProcedure,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dt = await connFunc.ExecuteDataTable("[MLU_sp_ST_Teacher_Update]", CommandType.StoredProcedure,
                           "@Name", SqlDbType.NVarChar, DataMain.Name.Replace("'", "").Trim(),
                           "@Avatar", SqlDbType.NVarChar, Avatar,
                           "@Alias", SqlDbType.NVarChar, DataMain.Alias.Replace("'", "").Trim(),
                           "@Code", SqlDbType.NVarChar, DataMain.Code.Replace("'", "").Trim(),
