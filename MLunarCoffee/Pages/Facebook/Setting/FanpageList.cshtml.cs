    ...
    public class FanpageListModel : PageModel
    {
         ...
         public async Task<IActionResult> OnPostLoadData()
        {
            try
            {
                ...
                {
                    ds = await confunc.ExecuteDataSet("[YYY_sp_Ticket_PageFacebook_Load]", CommandType.StoredProcedure,
         ...
         public async Task<IActionResult> OnPostDeleteItem(int id, string KeyPage)
        {
            try
            {
                ...
                {
                    connFunc.ExecuteDataTable("[YYY_sp_Ticket_PageFacebook_Delete]", CommandType.StoredProcedure,
         ...
         public async Task<IActionResult> OnPostUnregisterItem(int id, string KeyPage)
        {
            try
            {
                if (Comon.ComonFacebookXml.DeleteFileFTP_XML(KeyPage, _URL_Write_File_PageFB) == 1)
                {
                    ...
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        connFunc.ExecuteDataTable("[YYY_sp_Ticket_PageFacebook_Unregister]", CommandType.StoredProcedure,
                            "@CurrentID", SqlDbType.Int, id,
                            "@UserID", SqlDbType.Int, user.sys_userid
                        );
                    }
        ...
        private string UpdateLongKey(int PageId, string pageKey, string userLongToken)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                {
                    connFunc.ExecuteDataTable("[YYY_sp_Ticket_PageFacebook_Update_LongKey]", CommandType.StoredProcedure,
        ...
        private static int ActiveItem(int PageId, int Value)
        {
            try
            {
                using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                {
                    connFunc.ExecuteDataTable("[YYY_sp_PageFacebook_Active]", CommandType.StoredProcedure,
        ...
        private static async Task<int>  Check_Insert_Facebook_Mess(string page)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                {
                    dt =await connFunc.ExecuteDataTable("[YYY_sp_Facebook_v2_Checking_Data_Exist]", CommandType.StoredProcedure,
