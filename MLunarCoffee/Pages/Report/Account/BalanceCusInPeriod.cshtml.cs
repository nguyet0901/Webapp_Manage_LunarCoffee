...
{
    public class BalanceCusInPeriodModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadata(int branchID ,string dateFrom ,string dateTo ,string sersourceID)
        {
            try
            {
                ...
                {
                    dt = await confunc.ExecuteDataTable("[MLU_sp_Report_DebtCus_InPeriod]" ,CommandType.StoredProcedure ,
                        "@dateFrom" ,SqlDbType.DateTime ,Comon.Comon.DateTimeDMY_To_YMD(dateFrom)
                       ,"@dateTo" ,SqlDbType.DateTime ,Comon.Comon.DateTimeDMY_To_YMD(dateTo)
                       ,"@branchID" ,SqlDbType.Int ,branchID
                       ,"@sersourceID" ,SqlDbType.Int ,Convert.ToInt32(sersourceID ?? "0")
                       );
                }
                if (dt != null)
                {
        ...
        public async Task<IActionResult> OnPostLoadDataDetail(int branchID ,int custID ,string dateFrom ,string dateTo ,string sersourceID)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataTable("[MLU_sp_Report_DebtCus_InPeriod_Detail]" ,CommandType.StoredProcedure ,
                        "@dateFrom" ,SqlDbType.DateTime ,Comon.Comon.DateTimeDMY_To_YMD(dateFrom)
                       ,"@dateTo" ,SqlDbType.DateTime ,Comon.Comon.DateTimeDMY_To_YMD(dateTo)
                       ,"@branchID" ,SqlDbType.Int ,branchID
