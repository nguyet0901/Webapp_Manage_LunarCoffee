    ...
    public class LaboGeneralModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadata(int branchID, string dateFrom, string dateTo)
        {
            try
            {
                ...
                {
                    ds = await confunc.ExecuteDataSet("[MLU_sp_Report_Labo_v2]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostLoadataDetail(string branchID, string docID, string supID, string dateFrom, string dateTo)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataTable("[MLU_sp_Report_Labo_detail_v2]", CommandType.StoredProcedure
