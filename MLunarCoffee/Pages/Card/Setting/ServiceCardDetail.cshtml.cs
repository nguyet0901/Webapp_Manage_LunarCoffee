    ...
    public class ServiceCardDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostInitialize()
        {
            try
            {
                ...
                {
                    ds = await confunc.ExecuteDataSet("[MLU_sp_ServiceCard_LoadService]", CommandType.StoredProcedure);
                }
                if (ds != null)
                {
        ...
        public async Task<IActionResult> OnPostLoadata(int id)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    ...
                    ds = await confunc.ExecuteDataSet("[MLU_sp_ServiceCard_LoadDetail]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostDeleteItem(int id)
        {
            try
            {
                ...
                {
                    await connFunc.ExecuteDataTable("[MLU_sp_ServiceCard_Delete]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostExcuteData(string data, string CurrentID)
        {
            try
            {
                ...
                if (CurrentID == "0")
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dtResult = await connFunc.ExecuteDataTable("MLU_sp_ServiceCardDetail_Insert", CommandType.StoredProcedure,
                ...
                else
                {
                    ...
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dtResult = await connFunc.ExecuteDataTable("MLU_sp_ServiceCardDetail_Update", CommandType.StoredProcedure,
                            "@Name", SqlDbType.NVarChar, DataMain.NameServiceCard.Replace("'", "").Trim(),
                            "@Price_Sale", SqlDbType.Decimal, DataMain.Price_Sale,
                            "@Price_Use", SqlDbType.Decimal, DataMain.Price_Use,
                            "@AmountConsult", SqlDbType.Decimal, DataMain.AmountConsult,
