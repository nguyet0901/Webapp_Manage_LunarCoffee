...
{
    public class ServiceCardCashbackModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadInitialize(int CurrentID)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataTable("MLU_sp_Setting_Service_Card_LoadRule", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostExecute(int CurrentID, string data)
        {
            try
            {
                ...
                {
                    dt = await connFunc.ExecuteDataTable("[MLU_sp_Setting_Service_Card_UpdateRule]", CommandType.StoredProcedure,
                        "@CurrentID", SqlDbType.Int, CurrentID,
                        "@UserID", SqlDbType.Int, user.sys_userid,
                        "@Rule", SqlDbType.NVarChar, data,
                        "@Type", SqlDbType.NVarChar, "CASHBACK"
                    );
                }
                if (dt != null)
