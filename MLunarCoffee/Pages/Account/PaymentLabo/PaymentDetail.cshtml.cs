    ...
    public class PaymentDetail : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadInitialize(int supid ,int currentid)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataTable("[YYY_sp_Account_LaboFormGeneral]" ,CommandType.StoredProcedure
        ...
        public async Task<IActionResult> OnPostLoadToPay(int supid ,int beginid)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                {
                    ...
                    dt = await connFunc.ExecuteDataTable("[YYY_sp_Account_LaboFormToPay]" ,CommandType.StoredProcedure
                        ,"@SupID" ,SqlDbType.Int ,Convert.ToInt32(supid)
        ...
        public async Task<IActionResult> OnPostLoadToUpdate(int detailid)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                {
                    ds = await connFunc.ExecuteDataSet("[YYY_sp_Account_LaboFormView]" ,CommandType.StoredProcedure
            ...
            ,string dateCreated ,string content ,string amount ,string sign ,int branchID)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                {
                    if (CurrentID == "0")
                    {
                        dt = await connFunc.ExecuteDataTable("YYY_sp_Account_LaboPay" ,CommandType.StoredProcedure ,
                    ...
                    {
                        dt = await connFunc.ExecuteDataTable("YYY_sp_Account_LaboUpdate" ,CommandType.StoredProcedure ,
        ...
        public async Task<IActionResult> OnPostDelete(int detailid)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                {
                    dt = await connFunc.ExecuteDataTable("[YYY_sp_Account_LaboDelete]" ,CommandType.StoredProcedure
                       ,"@detailid" ,SqlDbType.Int ,Convert.ToInt32(detailid));
                }
                if (dt != null)
                {
