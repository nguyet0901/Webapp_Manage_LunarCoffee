    ...
    public class ProfileModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadCustomer(int CustID, int AppID ,int RoomID)
        {
            try
            {
                ...
                {
                    dt = await confunc.ExecuteDataTable("[MLU_sp_Customer_ScheduleInfo]" ,CommandType.StoredProcedure
        ...
        public async Task<IActionResult> OnPostUpdateStatus(int AppID  ,int RoomID, int Type, int Status)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataTable("[MLU_sp_Customer_ScheduleRealtime_UpdateStatus]" ,CommandType.StoredProcedure
