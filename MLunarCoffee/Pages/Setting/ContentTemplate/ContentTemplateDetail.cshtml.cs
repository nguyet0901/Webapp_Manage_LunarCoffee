    ...
    public class ContentTemplateDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadInitializeData()
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    ...
                    dt = await confunc.ExecuteDataTable("[MLU_sp_ContentTemplateMaster_LoadCombo]", CommandType.StoredProcedure);
        ...
        public async Task<IActionResult> OnPostLoadata(int CurrentID)
        {
            ...
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataTable("MLU_sp_Content_Detail_Load", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostExcuteDataDetail(string data, string CurrentID, string TypeStatusID)
        {
            try
            {
                ...
                if (CurrentID == "0")
                {
                    ...
                    {
                        DataTable dt = await connFunc.ExecuteDataTable("[MLU_sp_Content_Detail_Insert]", CommandType.StoredProcedure,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        DataTable dt = await connFunc.ExecuteDataTable("[MLU_sp_Content_Detail_Update]", CommandType.StoredProcedure,
                          "@DetailID", SqlDbType.Int, CurrentID,
                          "@MasterID", SqlDbType.Int, DataMain.MasterID,
                          "@Name", SqlDbType.NVarChar, DataMain.Name.Replace("'", "").Trim(),
                          "@Content", SqlDbType.NVarChar, DataMain.Content.Replace("'", "").Trim(),
