    ...
    public class PatientRecordImageDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadComboMain()
        {
            try
            {
                ...
                {
                    dt = await confunc.ExecuteDataTable("[MLU_sp_print_v2_GetTempleteByType]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostLoadDetail(string CurrentID)
        {
            ...
            using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
            {
                dt = await confunc.ExecuteDataTable("[MLU_sp_Setting_PatientRecord_LoadDetail]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostExcuteData(string Image,string Name, string CurrentID, string FormID)
        {
            try
            {
                ...
                if (CurrentID != "0")
                {
                    ...
                    {
                        DataTable dt = await connFunc.ExecuteDataTable("[MLU_sp_Setting_PatientRecord_UpdateImage]", CommandType.StoredProcedure,
