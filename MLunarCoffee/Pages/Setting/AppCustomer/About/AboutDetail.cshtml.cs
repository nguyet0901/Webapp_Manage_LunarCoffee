...
namespace MLunarCoffee.Pages.Setting.AppCustomer.About
{
    public class AboutDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadDetail(string CurrentID)
        {
            ...
            {
                dt = await confunc.ExecuteDataTable("[MLU_sp_Setting_AC_AboutUsPerson_LoadDetail]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostExcuteData(string data, string CurrentID)
        {
            try
            {
                ...
                if (CurrentID == "0")
                {
                    ...
                    {
                        DataTable dt = await connFunc.ExecuteDataTable("[MLU_sp_Setting_AC_AboutUsPerson_Insert]", CommandType.StoredProcedure,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        DataTable dt = await connFunc.ExecuteDataTable("[MLU_sp_Setting_AC_AboutUsPerson_Update]", CommandType.StoredProcedure,
                              "@Name ", SqlDbType.NVarChar, DataMain.Name.Replace("'", "").Trim(),
                              "@Image", SqlDbType.NVarChar, DataMain.Image,
                              "@UserID", SqlDbType.Int, user.sys_userid,
