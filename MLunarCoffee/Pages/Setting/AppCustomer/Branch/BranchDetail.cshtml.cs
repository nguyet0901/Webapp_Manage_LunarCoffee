    ...
    public class BranchDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostInit()
        {
            try
            {
                ...
                {
                    dt = await connFunc.ExecuteDataTable("[MLU_sp_Branch_LoadCombo]", CommandType.StoredProcedure);
                }
        ...
        public async Task<IActionResult> OnPostLoadata(int CurrentID)
        {
            try
            {
                ...
                {
                    ds = await confunc.ExecuteDataSet("[MLU_sp_Setting_AC_Branch_LoadDetail]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostExcuteData(string data, string CurrentID, string ImageDelete)
        {
            try
            {
                ...
                if (CurrentID == "0")
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dt = await connFunc.ExecuteDataTable("[MLU_sp_Setting_AC_Branch_Insert]", CommandType.StoredProcedure,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        await connFunc.ExecuteDataTable("[MLU_sp_Setting_AC_Branch_Update]", CommandType.StoredProcedure,
                            "@Name", SqlDbType.NVarChar, DataMain.Name.Replace("'", "").Trim(),
                            "@TimeOpen ", SqlDbType.NVarChar, DataMain.TimeOpen,
                            "@Address ", SqlDbType.NVarChar, DataMain.Address.Replace("'", "").Trim(),
                            "@Lat", SqlDbType.Float, DataMain.Lat,
