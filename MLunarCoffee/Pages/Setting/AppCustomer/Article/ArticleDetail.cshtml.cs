...
namespace MLunarCoffee.Pages.Setting.AppCustomer.Article
{
    public class ArticleDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadDetail(string CurrentID)
        {
            ...
            {
                dt = await confunc.ExecuteDataTable("[MLU_sp_Setting_AC_Article_LoadDetail]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostExcuteData(string data, string CurrentID)
        {
            try
            {
                ...
                if (CurrentID == "0")
                {
                    ...
                    {
                        DataTable dt = await connFunc.ExecuteDataTable("[MLU_sp_Setting_AC_Article_Insert]", CommandType.StoredProcedure,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        DataTable dt = await connFunc.ExecuteDataTable("[MLU_sp_Setting_AC_Article_Update]", CommandType.StoredProcedure,
