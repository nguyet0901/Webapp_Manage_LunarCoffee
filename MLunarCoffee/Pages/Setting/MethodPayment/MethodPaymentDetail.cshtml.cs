    ...
    public class MethodPaymentDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadata(int id)
        {
            try
            {
                ...
                {
                    dt = await confunc.ExecuteDataTable("[MLU_sp_MethodPayment_LoadDetail]" ,CommandType.StoredProcedure ,
        ...
        public async Task<IActionResult> OnPostLoadCombo()
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataTable("[MLU_sp_MethodPaymentTypeAll_LoadCombo]" ,CommandType.StoredProcedure);
                }
                if (dt != null)
                {
                    return Content(Comon.DataJson.Datatable(dt));
        ...
        public async Task<IActionResult> OnPostExcuteData(string data ,string CurrentID)
        {
            try
            {
                ...
                if (CurrentID == "0")
                {
                    ...
                    {
                        DataTable dt = await connFunc.ExecuteDataTable("[MLU_sp_MethodPayment_Insert]" ,CommandType.StoredProcedure ,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        DataTable dt = await connFunc.ExecuteDataTable("[MLU_sp_MethodPayment_Update]" ,CommandType.StoredProcedure ,
                            "@Name" ,SqlDbType.NVarChar ,DataMain.Name.Replace("'" ,"").Trim() ,
                            "@Type" ,SqlDbType.Int ,DataMain.Type ,
                            "@Note" ,SqlDbType.NVarChar ,Regex.Replace(DataMain.Note ,"\"|\'" ,"") ,
                            "@UserID" ,SqlDbType.Int ,user.sys_userid ,
