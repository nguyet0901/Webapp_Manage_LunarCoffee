    ...
    public class TreatmentStageDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadata(int id)
        {
            try
            {
                ...
                {
                    ds = await confunc.ExecuteDataSet("[YYY_sp_TreatmentStage_LoadDetail]" ,CommandType.StoredProcedure ,
        ...
        public async Task<IActionResult> OnPostExcuteData(string data ,string dataCons ,string CurrentID)
        {
            try
            {
                ...
                if (CurrentID.Trim() == "0")
                {
                    ...
                    {
                        DataTable dt = await connFunc.ExecuteDataTable("[YYY_sp_TreatmentStage_Insert]" ,CommandType.StoredProcedure ,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        DataTable dt = await connFunc.ExecuteDataTable("[YYY_sp_TreatmentStage_Update]" ,CommandType.StoredProcedure ,
                            "@Name" ,SqlDbType.NVarChar ,DataMain.Name.Replace("'" ,"").Trim() ,
                            "@DataCons" ,SqlDbType.Structured ,DataCons.Rows.Count > 0 ? DataCons : null ,
