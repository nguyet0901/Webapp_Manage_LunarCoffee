    ...
    {        
            ...
            try
            {
                ...
                tasks.Add(Task.Run(async () =>
                {
                    using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                    {
                        ...
                        dtType = await confunc.ExecuteDataTable("[MLU_sp_DiseaseStatus_LoadType]", CommandType.StoredProcedure);
                ...
                tasks.Add(Task.Run(async () =>
                {
                    using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                    {
                        ...
                        dtSignID = await confunc.ExecuteDataTable("[MLU_sp_DiseaseStatus_LoadComboSign]", CommandType.StoredProcedure);
        ...
        public async Task<IActionResult> OnPostLoadata(int id, int IsTeeth)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataSet("[MLU_sp_DiseaseStatus_LoadDetail]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostExcuteData(string data, string CurrentID, int IsTeeth)
        {
            try
            {
                ...
                if (CurrentID.Trim() == "0")
                {
                    ...
                    {
                        DataTable dt = await connFunc.ExecuteDataTable("[MLU_sp_DiseaseStatus_Insert]", CommandType.StoredProcedure,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        DataTable dt = await connFunc.ExecuteDataTable("[MLU_sp_DiseaseStatus_Update]", CommandType.StoredProcedure,
                            "@Name", SqlDbType.NVarChar, DataMain.Name.Replace("'", "").Trim(),
                            "@Content", SqlDbType.NVarChar, DataMain.Content.Replace("'", "").Trim(),
                            "@Color", SqlDbType.NVarChar, DataMain.Color.Replace("'", "").Trim(),
                            "@Type", SqlDbType.NVarChar, DataMain.Type.Replace("'", "").Trim(),
