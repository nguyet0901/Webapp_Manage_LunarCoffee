    ...
    public class GroupListModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostDataAreaLoad(string CurrentID)
        {
            try
            {
                ...
                {
                    dt = await connFunc.ExecuteDataTable("[MLU_sp_Setting_ICDArea_LoadData]" ,CommandType.StoredProcedure
        ...
        public async Task<IActionResult> OnPostDataCodeLoad()
        {
            try
            {
                ...
                using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                {
                    dt = await connFunc.ExecuteDataTable("[MLU_sp_Setting_ICDChapter_GetCode]" ,CommandType.StoredProcedure
                    );
                }
                return Content(Comon.DataJson.Datatable(dt));
            }
        ...
        public async Task<IActionResult> OnPostExcuteData(string chaptercode ,string data)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                {
                    dt = await connFunc.ExecuteDataTable("[MLU_sp_Setting_ICDChapter_Update]" ,CommandType.StoredProcedure ,
        ...
        public async Task<IActionResult> OnPostRemoveData(string chaptercode)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                {
                    dt = await connFunc.ExecuteDataTable("[MLU_sp_Setting_ICDChapter_Remove]" ,CommandType.StoredProcedure ,
                        "@chaptercode" ,SqlDbType.NVarChar ,chaptercode
                        ,"@UserID" ,SqlDbType.Int ,user.sys_userid
                    );
                }
