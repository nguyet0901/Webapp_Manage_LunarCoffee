    ...
    public class APITemplateDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadData(int id)
        {
            try
            {
                ...
                {
                    dt = await confunc.ExecuteDataTable("[YYY_sp_API_Accounting_LoadSetting_Detail]" ,CommandType.StoredProcedure ,
        ...
        public async Task<IActionResult> OnPostExcuteData(string data ,string CurrentID)
        {
            try
            {
                ...
                if (CurrentID == "0")
                {
                    ...
                    {
                        DataTable dt = await connFunc.ExecuteDataTable("[YYY_sp_API_Accounting_LoadSetting_Insert]" ,CommandType.StoredProcedure ,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        ...
                        DataTable dt = await connFunc.ExecuteDataTable("[YYY_sp_API_Accounting_LoadSetting_Update]" ,CommandType.StoredProcedure ,
                            "@ParaName" ,SqlDbType.NVarChar ,DataMain.ParaName.Replace("'" ,"").Trim() ,
                            "@MapField" ,SqlDbType.NVarChar ,DataMain.MapField.Replace("'" ,"").Trim() ,
                            "@IsMaster" ,SqlDbType.Int ,Convert.ToInt32(DataMain.IsMaster) ,
                            "@UseDataFirst" ,SqlDbType.Int ,Convert.ToInt32(DataMain.UseDataFirst) ,
        ...
        public async Task<IActionResult> OnPostDelete(string CurrentID)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                {
                    DataTable dt = await connFunc.ExecuteDataTable("[YYY_sp_API_Accounting_LoadSetting_Delete]" ,CommandType.StoredProcedure ,
                        "@CurrentID" ,SqlDbType.Int ,Convert.ToInt32(CurrentID) ,
