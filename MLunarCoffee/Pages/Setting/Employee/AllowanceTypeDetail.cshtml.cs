...
namespace MLunarCoffee.Pages.Setting.Employee.AllowanceTypeDetail
{
    public class AllowanceTypeDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadDetail(string CurrentID)
        {
            ...
            {
                dt = await confunc.ExecuteDataTable("[MLU_sp_Setting_AllowanceType_LoadDetail]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostExcuteData(string data, string CurrentID)
        {
            try
            {
                ...
                if (CurrentID == "0")
                {
                    ...
                    {
                        DataTable dt = await connFunc.ExecuteDataTable("[MLU_sp_Setting_AllowanceType_Insert]", CommandType.StoredProcedure,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        DataTable dt = await connFunc.ExecuteDataTable("[MLU_sp_Setting_AllowanceType_Update]", CommandType.StoredProcedure,
                              "@Name ", SqlDbType.NVarChar, DataMain.Name.Replace("'", "").Trim(),
                              "@UserID", SqlDbType.Int, user.sys_userid,
