    ...
    public class TicketList : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadCombo(string userID)
        {
            try
            {
                ...
                {
                    dts = await confunc.ExecuteDataTable("[MLU_sp_Customer_Type_ComboLoad]" ,CommandType.StoredProcedure);
                    dts.TableName = "Source";
                    ds.Tables.Add(dts);
                }
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dtsd = await confunc.ExecuteDataTable("[MLU_sp_Ticket_Combo_StatusData]" ,CommandType.StoredProcedure);
                    dtsd.TableName = "StatusData";
                    ds.Tables.Add(dtsd);
                }
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dtTele = await confunc.ExecuteDataTable("[MLU_sp_Ticket_LoadTele]" ,CommandType.StoredProcedure ,"@UserID" ,SqlDbType.Int ,userID);
                    dtTele.TableName = "Tele";
                    ds.Tables.Add(dtTele);
                }
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dtPerTele = await confunc.ExecuteDataTable("[MLU_sp_Check_Tele_Permission]" ,CommandType.StoredProcedure
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dtPerTele = await confunc.ExecuteDataTable("[MLU_sp_Ticket_GroupLoadCombo]" ,CommandType.StoredProcedure);
        ...
        public async Task<IActionResult> OnPostLoadList(string data ,int TicketID ,string idbegin ,string limit ,string LevelUser ,string GroupUser)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataTable("[MLU_sp_v2_Ticket_TicketList_Load]" ,CommandType.StoredProcedure ,
