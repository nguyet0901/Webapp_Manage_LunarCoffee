...
namespace MLunarCoffee.Pages.Employee
{
    public class UserDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadata(int id)
        {
            ...
            {
                dt = await confunc.ExecuteDataTable("[MLU_sp_User_Detail]" ,CommandType.StoredProcedure ,
        ...
        public async Task<IActionResult> OnPostLoadComboMain(int CurrentID)
        {
            try
            {
                ...
                {
                    DataTable dt = new DataTable();
                    dt = await confunc.ExecuteDataTable("MLU_sp_User_LoadCombo_GroupUser" ,CommandType.StoredProcedure);
                ...
                {
                    DataTable dt = new DataTable();
                    dt = await confunc.ExecuteDataTable("MLU_sp_User_LoadCombo_Employee" ,CommandType.StoredProcedure
        ...
        public async Task<IActionResult> OnPostExcuteData(string data ,string CurrentID)
        {
            try
            {
                ...
                if (CurrentID == "0")
                {
                    ...
                    {
                        var dt = await connFunc.ExecuteDataTable("[MLU_sp_User_Insert]" ,CommandType.StoredProcedure ,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        var dt = await connFunc.ExecuteDataTable("MLU_sp_User_Update" ,CommandType.StoredProcedure ,
                        "@Group_ID" ,SqlDbType.Int ,DataMain.Group ,
                        "@Branch_ID" ,SqlDbType.Int ,DataMain.Branch_ID ,
                        "@AreaBranch" ,SqlDbType.NVarChar ,DataMain.AreaBranch ,
                        "@isPassIP" ,SqlDbType.Int ,DataMain.isPassIP ,
