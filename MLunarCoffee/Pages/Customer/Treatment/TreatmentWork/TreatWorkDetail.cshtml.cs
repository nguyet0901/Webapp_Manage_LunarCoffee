    ...
    public class TreatWorkDetailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadata(int id)
        {
            try
            {
                ...
                {
                    dt = await confunc.ExecuteDataTable("[YYY_sp_Customer_Treatment_Work_LoadDetail]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostLoadComboMain()
        {
            try
            {
                ...
                tasks.Add(Task.Run(async () =>
                {
                    ...
                    {
                        DataTable dt = new DataTable();
                        dt = await confunc.ExecuteDataTable("[YYY_sp_Treatment_Task_Load]", CommandType.StoredProcedure);
                        dt.TableName = "Task";
                        return dt;
                    }
                }));
        ...
        public async Task<IActionResult> OnPostExcuteData(string data, string CurrentID)
        {
            try
            {
                ...
                if (CurrentID == "0")
                {
                    ...
                    {
                        dt = await connFunc.ExecuteDataTable("[YYY_sp_Customer_Treatment_Work_Insert]", CommandType.StoredProcedure,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dt = await connFunc.ExecuteDataTable("[YYY_sp_Customer_Treatment_Work_Update]", CommandType.StoredProcedure,
                            "@EmployeeID", SqlDbType.Int, DataMain.Rows[0]["Employee"],
                            "@Content", SqlDbType.NVarChar, DataMain.Rows[0]["Content"].ToString().Replace("'", "").Trim(),
                            "@Task", SqlDbType.NVarChar, DataMain.Rows[0]["Task"].ToString().Replace("'", "").Trim(),
