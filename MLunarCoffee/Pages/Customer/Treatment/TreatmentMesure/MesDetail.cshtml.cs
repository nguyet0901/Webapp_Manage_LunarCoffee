...
namespace MLunarCoffee.Pages.Customer.Treatment.TreatmentMesure
{
    ...
    public class MesDetail : PageModel
    {
            ...
            try
            {
                ...
                tasks.Add(Task.Run(async () =>
                {
                    using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                    {
                        ...
                        dt = await confunc.ExecuteDataTable("YYY_sp_Customer_MeasureType", CommandType.StoredProcedure
                             );
                        dt.TableName = "Type";
                        return dt;
                    }
        ...
        public async Task<IActionResult> OnPostLoadataDetail(string CurrentID, string CustomerID)
        {
            ...
            using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
            {
                ds = await confunc.ExecuteDataSet("[YYY_sp_Customer_MeasureDetail]", CommandType.StoredProcedure,
        ...
        public async Task<IActionResult> OnPostDeleteItem(string CustomerID, string currentid)
        {
            try
            {
                ...
                {
                    await connFunc.ExecuteDataTable("[YYY_sp_Customer_MeasureDelete]", CommandType.StoredProcedure,
                        "@CurrentID", SqlDbType.Int, currentid,
        ...
        public async Task<IActionResult> OnPostExcuteData(string CurrentID, string CustomerID, string TreatmentID, string data)
        {
            try
            {
                ...
                if (CurrentID == "0")
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dtResult = await connFunc.ExecuteDataTable("[YYY_sp_Customer_Measure_Insert]", CommandType.StoredProcedure,
                            "@CustomerID", SqlDbType.Int, CustomerID,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dtResult = await connFunc.ExecuteDataTable("[YYY_sp_Customer_Measure_Update]", CommandType.StoredProcedure,
                            "@CustomerID", SqlDbType.Int, CustomerID,
                            "@CurrentID", SqlDbType.Int, CurrentID,
                            "@BranchID", SqlDbType.NVarChar, user.sys_branchID,
                            "@data", SqlDbType.Structured, dt.Rows.Count > 0 ? dt : null,
