    ...
    public class ImageNewFolderModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadata(int id)
        {
            try
            {
                ...
                {
                    dt = await confunc.ExecuteDataTable("[MLU_sp_CustomerImage_Folder_LoadDetail]", CommandType.StoredProcedure,
            ...
            try
            {
                ...
                if (CurrentID == "0")
                {
                    if (Comon.Comon.CheckDirectoryFTP(CustomerID, FolderName))
                    {
                        ...
                        {
                            await connFunc.ExecuteDataTable("[MLU_sp_CustomerImage_Create_Folder]", CommandType.StoredProcedure,
                ...
                else
                {
                    if (Comon.Comon.UpdateDirectoryFTP(CustomerID, oldName, FolderName))
                    {
                        using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                        {
                            await connFunc.ExecuteDataTable("[MLU_sp_CustomerImage_Update_Folder]", CommandType.StoredProcedure,
