    ...
    public class TabDepositDetail_ReturnModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadInitialize(string CustomerID)
        {
            try
            {
                ...
                {
                    DataTable dt = new DataTable();
                    dt = await confunc.ExecuteDataTable("MLU_sp_Customer_Desposit_Reason_Return" ,CommandType.StoredProcedure);
                ...
                {
                    DataTable dt = new DataTable();
                    dt = await confunc.ExecuteDataTable("MLU_sp_Customer_Deposit_DetectMaxDeposit_Return" ,CommandType.StoredProcedure
        ...
        public async Task<IActionResult> OnPostLoadata(int id)
        {
            try
            {
                ...
                using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                {
                    dt = await confunc.ExecuteDataTable("MLU_sp_Customer_Deposit_LoadDetail" ,CommandType.StoredProcedure ,
        ...
        public async Task<IActionResult> OnPostExcuteData(string data ,string CustomerID ,string CurrentID)
        {
            try
            {
                ...
                if (CurrentID == "0")
                {
                    ...
                    {
                        dtCus = await connFunc.ExecuteDataTable("MLU_sp_Customer_Deposit_Insert" ,CommandType.StoredProcedure ,
                ...
                else
                {
                    //using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    //{
                    //    dtCus = await connFunc.ExecuteDataTable("MLU_sp_Customer_Deposit_Update", CommandType.StoredProcedure,
                    //        "@CustomerID", SqlDbType.Int, CustomerID,
                    //        "@Method", SqlDbType.Int, DataMain.Method,
                    //        "@ReasonReturn", SqlDbType.Int, DataMain.ReasonReturn,
