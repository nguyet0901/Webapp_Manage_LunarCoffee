    ...
    public class DentistChartCanvas : PageModel
    {
        ...
        public async Task<IActionResult> OnPostLoadIni(string Type)
        {
            ...
            tasks.Add(Task.Run(async () =>
                {
                    using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                    {
                        ...
                        dt = await confunc.ExecuteDataTable("[YYY_sp_DentistChart_LoadTeeth]" ,CommandType.StoredProcedure
            ...
            tasks.Add(Task.Run(async () =>
            {
                ...
                {
                    DataTable dt = new DataTable();
                    dt = await confunc.ExecuteDataTable("[YYY_sp_DentistChart_LoadClassi]" ,CommandType.StoredProcedure);
                    dt.TableName = "Classi";
                    return dt;
                }
            }));
            tasks.Add(Task.Run(async () =>
                {
                    ...
                    {
                        DataTable dt = new DataTable();
                        dt = await confunc.ExecuteDataTable("[YYY_sp_DentistChart_LoadDisease]" ,CommandType.StoredProcedure);
            ...
            tasks.Add(Task.Run(async () =>
              {
                  ...
                  {
                      DataTable dt = new DataTable();
                      dt = await confunc.ExecuteDataTable("[YYY_sp_DentistChart_LoadImage]" ,CommandType.StoredProcedure);
        ...
        public async Task<IActionResult> OnPostLoadDetail(string currentid)
        {
            ...
            using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
            {
                ds = await confunc.ExecuteDataSet("[YYY_sp_DentistChart_LoadDetail]" ,CommandType.StoredProcedure
        ...
        public async Task<IActionResult> OnPostExecute(string data ,string CurrentID ,string dataDetail)
        {
            try
            {
                ...
                if (CurrentID == "0")
                {
                    ...
                    {
                        dt = await connFunc.ExecuteDataTable("YYY_sp_DentistChart_Insert" ,CommandType.StoredProcedure ,
                ...
                else
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        dt = await connFunc.ExecuteDataTable("YYY_sp_DentistChart_Update" ,CommandType.StoredProcedure ,
                            "@CurrentID" ,SqlDbType.Int ,Convert.ToInt32(CurrentID) ,
                            "@Note" ,SqlDbType.NVarChar ,!String.IsNullOrEmpty(dtDetail.Note) ? dtDetail.Note : "" ,
                            "@Subclinical" ,SqlDbType.NVarChar ,!String.IsNullOrEmpty(dtDetail.Subclinical) ? dtDetail.Subclinical : "" ,
                            "@Diagnose" ,SqlDbType.NVarChar ,!String.IsNullOrEmpty(dtDetail.Diagnose) ? dtDetail.Diagnose : "" ,
