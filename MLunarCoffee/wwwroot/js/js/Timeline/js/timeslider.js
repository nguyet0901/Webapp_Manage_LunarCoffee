...
(function ($) {
    ...
    TimeSlider.prototype.init = function (element, options) {
        ...
        this.$element.append(
            ...
            '<div class="prompts" style="top:-' + (this.$element.height() * 2 + this.gt_height + 100) + 'px;"></div>'
        );
    ...
    TimeSlider.prototype.add_graduations = function () {
        ...
        for (var i = -4; i <= num_steps; i++) {
            ...
            left = i * px_per_step * 1.5 + this.px_per_ms * ms_offset;
    ...
    TimeSlider.prototype.add_cell = function (timecell) {
        ...
        if (!this.$ruler.find('#' + timecell['_id']).length) {
            ...
            this.$prompts.append(
                ...
                '<div class="body">'
                + this.timestamp_to_date(timecell['start'])
                + '</div>' +
                '</div>' +
                (timecell['stop'] ?
                    ...
                    '<div class="triangle-up"></div>' +
                    '<div class="body">'
                    + this.timestamp_to_date(timecell['stop'])
                    + '</div>' +
                    '</div>'
                    : '')
            );
    ...
    TimeSlider.prototype.set_time_caret_position = function () {
        this.time_caret.css('left', (this.options.current_timestamp - this.options.start_timestamp) * this.px_per_ms * 1.5);
    };
    ...
    TimeSlider.prototype.set_new_start_timestamp = function (timestamp) {
        this.set_ruler_position((this.options.start_timestamp - timestamp) * this.px_per_ms * 1.5);
    };
    ...
    TimeSlider.prototype._edit_time_cell = function (options) {
        ...
        if (has_start) {
            ...
            var left = (options.start - this.options.start_timestamp) * this.px_per_ms * 1.5;
            var width = ((stop !== null ? stop : this.options.current_timestamp) - options.start) * this.px_per_ms * 1.5;
        ...
        if (has_stop) {
            ...
            var left = (start - this.options.start_timestamp) * this.px_per_ms * 1.5;
            var width = (options.stop - start) * this.px_per_ms * 1.5;
