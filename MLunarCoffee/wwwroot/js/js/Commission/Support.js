...
function RevenueSupport({
    ...
    this.RevenueDetail_Count = async function (data) {
        new Promise((resolve, reject) => {
            ...
            if (data && data.length > 0) {
                for (let i = 0; i < data.length; i++) {
                    let RePercent = data[i].Amount != 0 ? 100 : data[i].Percent;
                    commission = data[i].Amount != 0
                        ? data[i].Amount
                        : ((sys_dencos_Main == 0 && data[i].TimeTreat != 0)
                            ? (data[i].Price / data[i].TimeTreat)
                            : (data[i].Price));
    ...
    this.ExportDetail = async function (filename) {
        ...
        let dataExport = {
            ...
            , "CusCodeOld": Outlang["Ma_khach_hang_cu"] != undefined ? Outlang["Ma_khach_hang_cu"] : 'Mã khách hàng cũ'
            ...
            , "Service_Code": {
                dataNamePer: 'SerCode',
                data: [Outlang["Ma_dich_vu"]]
            }
            , "ServiceID": [Outlang["Dich_vu"], (v) => {return settings.RevenueDetail_ServiceName(v)}]
            , "ServiceType": {
                dataNamePer: 'ServiceType',
                data: [Outlang["Loai_dich_vu"]]
            }
            , "SalesCode": {
                dataNamePer: 'SalesCode',
                data: [Outlang["Ma_len_don"]]
            }
            , "Quantity": Outlang["So_luong"]
            , "Price": {
                dataNamePer: 'PriceSer',
                data: [Outlang["Gia_dich_vu"]]
            }
