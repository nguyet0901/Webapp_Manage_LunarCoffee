function RevenueAssistant({
    ...
    this.RevenueMaster_Count = async function (data, datacom) {
        new Promise((resolve, reject) => {
            ...
            let { IsAssGreater, IsAssDevide } = settings;
            ...
            if (datacom && datacom.length > 0) {
                for (let i = 0; i < datacom.length; i++) {
                    if (datacom[i].EMP1 != 0) {
                        ...
                        e.paymentperiod = datacom[i].TotalCurrentPaid;
                        maindt.push(e);
                    }
                    if (datacom[i].EMP2 != 0) {
                        let e = {};
            ...
            if (data && data.length > 0) {
                ...
                for (let i = 0; i < data.length; i++) {
                    let newpercent = (sys_dencos_Main == 1) ? data[i].NewPer : (data[i].TimeTreat != 0
                        ...
                        : 0);
                    let amountequalper = newpercent != 0 ? (data[i].TotalPaid * newpercent / 100) : 0;
                ...
                for (let i = 0; i < data.length; i++) {
                    if (Number(data[i].Havingrevenue) == 1) {
                        ...
                        if (data[i].PT1 != 0) {
                            ...
                            e.paymentperiod = data[i].TotalCurrentPaid;
                            e.tabid = data[i].TabID;
                            maindt.push(e);
                        }
                        if (data[i].PT2 != 0 && IsAssDevide == 1) {
                            let e = {};
                            e.emp = data[i].PT2;
                            e.realamount = realamount;
                            e.paymentperiod = data[i].TotalCurrentPaid;
            ...
            maindt.reduce(function (res, value) { 
                if (!res[value.emp]) {
                    stt = stt + 1;
                    res[value.emp] = { stt: stt, emp: value.emp, realamount: 0, empName: settings.RevenueDetail_EmpName(value.emp), paymentperiod:0 };
    ...
    this.RevenueDetail_Load = function (EmployeeID) {
        ...
        obj.branch = BranchID;
        XhrRevenueLoad = AjaxJWT(url = "/api/Commission/RevAssistant_LoadDetail"
            , data = JSON.stringify(obj)
            , async = true
            , success = function (result) {
                ...
                for (let i = 0; i < data.length; i++) {
                    if (data[i].Manual_Revenue == 0) {
                        if (Number(data[i].Havingrevenue) == 1) {
                            ...
                            if (CurrentMaster != 0) {
                                if (data[i].PT1 == CurrentMaster) settings.RevenueDetail_Push(data[i], "PT1", "RealAmount1", commission, realamount);
                                if (data[i].PT2 == CurrentMaster && IsAssDevide == 1) settings.RevenueDetail_Push(data[i], "PT2", "RealAmount2", commission, realamount);
                                if (data[i].PT3 == CurrentMaster && IsAssDevide == 1) settings.RevenueDetail_Push(data[i], "PT3", "RealAmount3", commission, realamount);
                                if (data[i].PT4 == CurrentMaster && IsAssDevide == 1) settings.RevenueDetail_Push(data[i], "PT4", "RealAmount4", commission, realamount);
                            }
                            else {

                                if (data[i].PT1 != 0) settings.RevenueDetail_Push(data[i], "PT1", "RealAmount1", commission, realamount);
                                if (data[i].PT2 != 0 && IsAssDevide == 1) settings.RevenueDetail_Push(data[i], "PT2", "RealAmount2", commission, realamount);
                                if (data[i].PT3 != 0 && IsAssDevide == 1) settings.RevenueDetail_Push(data[i], "PT3", "RealAmount3", commission, realamount);
                                if (data[i].PT4 != 0 && IsAssDevide == 1) settings.RevenueDetail_Push(data[i], "PT4", "RealAmount4", commission, realamount);
                            }
                        }
                    }
                    else {
                        if (CurrentMaster != 0) {
                            if (data[i].PT1 == CurrentMaster) settings.RevenueDetail_Push(data[i], "PT1", "RealAmount1", commission, data[i].RealAmount1);
                            if (data[i].PT2 == CurrentMaster) settings.RevenueDetail_Push(data[i], "PT2", "RealAmount2", commission, data[i].RealAmount2);
                            if (data[i].PT3 == CurrentMaster) settings.RevenueDetail_Push(data[i], "PT3", "RealAmount3", commission, data[i].RealAmount3);

                        }
                        else {

                            if (data[i].PT1 != 0) settings.RevenueDetail_Push(data[i], "PT1", "RealAmount1", commission, data[i].RealAmount1);
                            if (data[i].PT2 != 0) settings.RevenueDetail_Push(data[i], "PT2", "RealAmount2", commission, data[i].RealAmount2);
                            if (data[i].PT3 != 0) settings.RevenueDetail_Push(data[i], "PT3", "RealAmount3", commission, data[i].RealAmount3);
