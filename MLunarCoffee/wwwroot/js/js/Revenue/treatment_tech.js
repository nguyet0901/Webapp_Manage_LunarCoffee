...
function Revenue_Executing_Method(dataset, branchid) {
    ...
    for (i = 0; i < datamain.length; i++) {
        if (datamain[i].Tech1 + datamain[i].Tech2 != 0) {
            ...
            datamain[i].NumPerson = ((datamain[i].Tech1 != 0) ? 1 : 0)
...
function RenderReportMaster_TimeJoin(empid) {
    ...
    for (_i = 0; _i < dataDetail.length; _i++) {
        if (dataDetail[_i].Tech1 == empid || dataDetail[_i].Tech2 == empid)
            timejoin = timejoin + 1;
    }
...
function RenderReportMaster_Amount(empid) {
    let amount = 0;
    for (_i = 0; _i < dataDetail.length; _i++) {
        if (dataDetail[_i].Tech1 == empid) amount = amount + dataDetail[_i].RealAmount1;
        if (dataDetail[_i].Tech2 == empid) amount = amount + dataDetail[_i].RealAmount2;
    }
    for (_i = 0; _i < dataDetail_manual.length; _i++) {
...
function ViewDetail(idDetail, items, amount) {
    let data = dataDetail.filter(word => word["Tech1"] == idDetail || word["Tech2"] == idDetail);
        ...
        if (data && data.length > 0) {
            for (let i = 0; i < data.length; i++) {
                ...
                let tr =
                    ...
                    + '<td>' + item.CustName + '</td>'
                    + '<td treatment_tech class="_tab_control_" data-tab="phone_customer">' + item.CustPhone + '</td>'
                    + '<td>'
                    ...
                    + formatNumber(
                        Math.ceil(
                            ((item.Tech1 != idDetail) ? 0 : item.RealAmount1)
                            +
                            ((item.Tech2 != idDetail) ? 0 : item.RealAmount2)
                        )
                    )
