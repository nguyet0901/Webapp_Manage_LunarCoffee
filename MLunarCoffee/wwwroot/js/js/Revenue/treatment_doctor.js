...
function Revenue_Executing_Method(dataset, branchid) {
    ...
    for (i = 0; i < datamain.length; i++) {
        if (datamain[i].BS1 + datamain[i].BS2 + datamain[i].BS3 + datamain[i].BS4 != 0) {
            ...
            datamain[i].NumPerson = ((datamain[i].BS1 != 0) ? 1 : 0)
...
function RenderReportMaster_TimeJoin(empid) {
    ...
    for (_i = 0; _i < dataDetail.length; _i++) {
        if (dataDetail[_i].BS1 == empid || dataDetail[_i].BS2 == empid || dataDetail[_i].BS3 == empid || dataDetail[_i].BS4 == empid)
            timejoin = timejoin + 1;
    }
...
function RenderReportMaster_Amount(empid) {
    let amount = 0;
    for (_i = 0; _i < dataDetail.length; _i++) {
        if (dataDetail[_i].BS1 == empid) amount = amount + dataDetail[_i].RealAmount1;
        if (dataDetail[_i].BS2 == empid) amount = amount + dataDetail[_i].RealAmount2;
        if (dataDetail[_i].BS3 == empid) amount = amount + dataDetail[_i].RealAmount3;
        if (dataDetail[_i].BS4 == empid) amount = amount + dataDetail[_i].RealAmount4;
    }
    for (_i = 0; _i < dataDetail_manual.length; _i++) {
...
function RenderReportMaster_PAID(empid) {
    let amount = 0;
    for (_i = 0; _i < dataDetail.length; _i++) {
        if (dataDetail[_i].BS1 == empid) amount = amount + dataDetail[_i].PaidInPeriod;
        if (dataDetail[_i].BS2 == empid) amount = amount + dataDetail[_i].PaidInPeriod;
        if (dataDetail[_i].BS3 == empid) amount = amount + dataDetail[_i].PaidInPeriod;
        if (dataDetail[_i].BS4 == empid) amount = amount + dataDetail[_i].PaidInPeriod;
    }
    return amount;
}
    ...
    if (myNode != null) {
        ...
        if (data && data.length > 0) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                let paidAmount = RenderReportMaster_PAID(item.ID);
...
function ViewDetail(idDetail, items, amount) {
    let data = dataDetail.filter(word => word["BS1"] == idDetail || word["BS2"] == idDetail || word["BS3"] == idDetail || word["BS4"] == idDetail);
        ...
        if (data && data.length > 0) {
            for (let i = 0; i < data.length; i++) {
                ...
                let tr =
                    ...
                    + '<td>' + Math.ceil(item.PerDiscount) + ' %'
                    + ((Number(item.CardAmountUsing) != 0)
                        ? ('<span class="statustreat_row"> Sử Dụng Thẻ: '
                            + formatNumber(item.CardAmountUsing)
                            + '</span>')
                        :""
                    )
                    + '</td>'
                    ...
                    + '<td style="text-align:center"><span class="money_row">'
                    + formatNumber(item.PaidInPeriod)
                    + '</span></td>'
                    + '<td>' + formatNumber(item.Price - item.TotalPay) + '</td>'

                    + '<td><span class="statustreat_row">'
                    + ((Number(sys_dencos_Main) == 1)
                        ? (item.Treat_Percent)
                        : Math.ceil(item.TimeTreatIndex / item.TimeTreat * 100)
                        )
                    + ' %</span></td>'
                    ...
                    + formatNumber(
                        Math.ceil(
                            ((item.BS1 != idDetail) ? 0 : item.RealAmount1)
                            +
                            ((item.BS2 != idDetail) ? 0 : item.RealAmount2)
                            +
                            ((item.BS3 != idDetail) ? 0 : item.RealAmount3)
                            +
                            ((item.BS4 != idDetail) ? 0 : item.RealAmount4)
                        )
                    )
                ...
                stringContent = stringContent
                    + ((Number(item.Before_Revenue) == 0) ? '<tr role="row" class="vt-number">' : '<tr role="row" class="vt-number outrangerow">')
                    + tr
                    + '</tr>';
            }
        }
