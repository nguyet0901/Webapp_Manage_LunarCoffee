...
function Checking_DataTab_IS_PageNotAllow(datablock) {
    try {
        if (datablock["MenuBlock"] == undefined || datablock["MenuBlock"].length == 0)
            return false;
        return datablock["MenuBlock"].includes(Master_CurrentPageLink);
    }
    catch (ex) {
        return false;
    }
...
function Checking_DataTab_IS_Allow (data, datatab) {
    ...
    if (data != undefined && Object.values(data).length != 0) {
        if (data[datatab] != undefined && !Checking_DataTab_IS_PageNotAllow(data[datatab])) {
            result = 1;
        }
    }
    return result;
