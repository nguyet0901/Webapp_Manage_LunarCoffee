...
function syslog_cutprofile (act, cust) {
    syslog_executed(act = act, val = '', type = 'ccp', cust = cust, content =  '')
}
...
//Employee
function syslog_emp(act, val) {
    syslog_executed(act = act, val = val, type = 'emp', cust = 0, content = '')
}

//User
function syslog_usr(act, val) {
    syslog_executed(act = act, val = val, type = 'usr', cust = 0, content = '')
}
...
function syslog_userlogin(act, val) {
    syslog_executed(act = act, val = val, type = 'ulg', cust = '', content = '')
}
...
async function syslog_executed (act, val, type, cust, content) {
            ...
            () => {
                try {
                    if (typeof sys_userID_Main !== 'undefined' && sys_userID_Main != ''
                        ...
                        e.Type = type;
                        e.Content = content != undefined ? content.substring(0, 100) : '';
                        e.Page = syslog_getpath()
                        e.Action = act;
                        e.Value = val.replaceAll('"', '').toString();
                        AjaxJWT(url = "/api/Log/Add"
