var VTT_TableToExcel;

(function ($) {
    let parseDomToTable = function (wb, ws, table, opts) {

        let _r, _c, cs, rs, r, c;
        let rows = [...table.getElementsByTagName("tr")];
        let widths = table.getAttribute("data-cols-width");
        let width = table.className.match(/excel-cols-width-\d+/gm);
        if (widths)
        ...
        for (_r = 0; _r < rows.length; ++_r) {
            ...
            for (_c = 0; _c < tds.length; ++_c) {
                ...
                let td = tds[_c];
                let _imagelink = td.getAttribute("datalink");
                if (_imagelink != undefined && _imagelink != null && _imagelink != "") {

                    var img = document.createElement("img");
                    img.src = _imagelink;
                    var myBase64Image = getBase64Image(img);
                    const imageId1 = wb.addImage({
                        base64: myBase64Image,
                        extension: _imagelink.includes('png') ? 'png' :'jpg',
                    });
                ...
                if (_r == 0) {
                    ...
                    //ws.columns[_c].width = Math.round(tds[_c].offsetWidth / 7.2) - 40; // convert pixel to character width
                }
            }
        }
    ...
    VTT_TableToExcel = {
        ...
        tableToSheet (wb, table, opts) {
            ...
            ws = parseDomToTable(wb, ws, table, opts);
            return wb;
        },
