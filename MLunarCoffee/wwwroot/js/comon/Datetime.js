...
function formatBirthNumber (year) {
    try {
        if (!isNaN(year)) {
            var x = Number(year);
...
function validBirthNumber (year) {
    try {
        if (year == "") return true;
        return formatBirthNumber(year) != 0;
...
function ConvertDateTimeUTC_NoYear (x) {
    ...
    try {
        ...
        if (d.getDate() == datenow.getDate() && d.getYear() == datenow.getYear() && d.getMonth() == datenow.getMonth()) {
            ...
            if (d.getYear() == datenow.getYear()) return date + " / " + month;
            else return date + "/" + month + "/" + d.getFullYear().toString().slice(-2);
        }
        //return d;
    }
    catch (err) {
...
function BigIntToDDMMYYYY (time) {
    try {
        time = time.toString();
        if (time == "" || time == "0") return "";
        let result = '';
