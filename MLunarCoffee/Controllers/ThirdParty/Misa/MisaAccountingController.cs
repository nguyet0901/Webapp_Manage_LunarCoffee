                ...
                if (BaseLink != null && BaseLink != "")
                {
                        ...
                        _config["Jwt:Issuer"].ToString() ,token))
                    {
                        ...
                        using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                        {
                            ...
                            dt = await confunc.ExecuteDataTable("MLU_sp_MisaAccounting_LoadData" ,CommandType.StoredProcedure
                                    ...
                                    if (ta != null)
                                    {
                                        tasks.Add(Task.Factory.StartNew(async () =>
                                        {
                                            using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                                            {
                                                ...
                                                dt = await confunc.ExecuteDataTable("MLU_sp_MisaAccounting_LoadDataMulti" ,CommandType.StoredProcedure
        ...
        private async Task<IActionResult> HandleCallBack(bool isDelete ,List<MisaCallBackParaDetail> data)
        {
            try
            {
                if (data != null)
                {
                    ...
                    {
                        dt = await connFunc.ExecuteDataTable("[MLU_sp_MisaAccounting_UpdateResult]" ,CommandType.StoredProcedure ,
