                        ...
                        _config["Jwt:Issuer"].ToString() ,token))
                    {
                        ...
                        var tasks = new List<Task<DataTable>>
                        {
                            Task.Run(async () =>
                            {
                                ...
                                {
                                    DataTable dt = new DataTable();
                                    dt = await confunc.ExecuteDataTable("[YYY_sp_API_Accounting_LoadSetting]" ,CommandType.StoredProcedure ,
                            ...
                            Task.Run(async () =>
                            {
                                ...
                                {
                                    DataTable dt = new DataTable();
                                    dt = await confunc.ExecuteDataTable("[YYY_sp_API_Accounting_LoadData]" ,CommandType.StoredProcedure
                                    ...
                                    if (ta != null)
                                    {
                                        tasks.Add(Task.Run(async () =>
                                        {
                                            using (Models.ExecuteDataBase confunc = new Models.ExecuteDataBase())
                                            {
                                                ...
                                                dt = await confunc.ExecuteDataTable("YYY_sp_API_Account_LoadDataMulti" ,CommandType.StoredProcedure
        ...
        private async Task HandleCallBack(bool isDelete ,List<CallbackResult> data)
        {
            try
            {
                if (data != null)
                {
                    ...
                    {
                        dt = await connFunc.ExecuteDataTable("[YYY_sp_API_Account_Callback]" ,CommandType.StoredProcedure ,
