...
namespace MLunarCoffee.Controllers.Data
{
    ...
    public class DistrictController : Controller
    {
        ...
        public DistrictController(IConfiguration config ,ITokenService tokenService)
        {
            _config = config;
        ...
        [Route("GetList")]
        [HttpPost]
        public async Task<IActionResult> GetGetList(District District)
        {
            try
            {
                    ...
                    _config["Jwt:Issuer"].ToString() ,token))
                {
                    ...
                    var content = new StringContent(JsonConvert.SerializeObject(District) ,Encoding.UTF8 ,"application/json");
                    using (HttpClient clientautho = new HttpClient() { BaseAddress = new Uri(BaseUrl) })
                    {
                        ...
                        var result = await clientautho.PostAsync("api/District/GetList" ,content);
