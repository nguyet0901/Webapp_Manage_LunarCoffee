    ...
    public class SMSController : ControllerBase
    {
        ...
        public async Task<IActionResult> SMSUpdateState(SMSSuccess sms)
        {
            try
            {
                using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                {
                    await connFunc.ExecuteDataTable("[MLU_sp_Customer_SMS_UpdateStatus]" ,CommandType.StoredProcedure ,
        ...
        public async Task<IActionResult> SMSCallback(SMSResultCallback sms)
        {
            try
            {
                ...
                if (Secret == Settings.Secret)
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        await connFunc.ExecuteDataTable("[MLU_sp_Customer_SMS_UpdateStatusv2]"
        ...
        public async Task<IActionResult> SMSMultiCallback(SMSResultCallbackMulti sms)
        {
            try
            {
                ...
                if (Secret == Settings.Secret)
                {
                    using (Models.ExecuteDataBase connFunc = new Models.ExecuteDataBase())
                    {
                        await connFunc.ExecuteDataTable("[MLU_sp_Customer_SMS_UpdateStatusmulti]"
                            ,CommandType.StoredProcedure ,
                             "@phone" ,SqlDbType.NVarChar ,sms.phone
