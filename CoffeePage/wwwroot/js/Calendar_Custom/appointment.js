...
function callvtt_temporary (uid, branch, time, date, resource) {
    note__CurrentID = uid;
    callvtt_temporary_resetField();
    callvtt_temporary_event();
...
function callvtt_temporary_resetField() {
...
function callvtt_temporary_event() {
...
function TempClose () {
    $('#vtt_master').removeClass('opacity-3 pe-none');
    $('#vtt_teamporaryarea').addClass('d-none');
    DetectFormAppear = 1;
            ...
            AjaxLoad(url = "/Appointment/Appointment/?handler=ExcuteTemp"
                ...
                , success = function (result) {
                    if (result != "0") {
                        ...
                        if (typeof update_cal_vtt_app !== 'undefined' && $.isFunction(update_cal_vtt_app)) {
                            update_cal_vtt_app(_d.ID, _d.Branch, Number(_d.DateFrom_Int), 0, 1, isupdate);
                        }
                        TempClose();
                    }
                }
