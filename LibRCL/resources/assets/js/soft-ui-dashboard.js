...
function navbarFixed (el) {
    let classes = ['position-sticky', 'blur', 'shadow-blur', 'left-auto', 'top-0', 'z-index-sticky'];
...
window.addEventListener('resize', function (event) {
    total.forEach(function (item, i) {
        if (item.querySelector('.moving-tab') != undefined) {
...
// Move Element User To Top

if (window.innerWidth < 991) {
    $("#Master_Top_User").prepend($(".master-item-top"))
}

if (window.innerWidth < 1200) {
    let bodyMain = document.getElementById('BodyMain');
    if (bodyMain)
        bodyMain.classList.add('g-sidenav-hidden');
}
